...
import { LayoutGrid, List, Menu, X, Trash2 } from 'lucide-react'
...
import { StatsBar, computeStats } from '@/components/StatsBar'
...
function applyFilters(entries: Entry[], filters: FilterState): Entry[] {
  return entries.filter((entry) => {
    ...
    // Pose family (scene-level)
    if (
      filters.poseFamilies.length > 0 &&
      !entry.scenes?.some((s) => filters.poseFamilies.includes(s.poseFamily))
    )
      return false
...
function applySort(entries: Entry[], field: SortField, dir: SortDir): Entry[] {
  const sorted = [...entries].sort((a, b) => {
    switch (field) {
      ...
      case 'review_state': {
        const order = { approved: 0, raw: 1, unset: 2 }
...
function CorpusExplorer() {
  ...
  const { totalScenes, avgScore } = useMemo(() => computeStats(filteredEntries), [filteredEntries])
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="relative z-10 w-64 h-full lg:h-auto">
            <FilterSidebar
              entries={entries}
              filters={filters}
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="sticky top-0 z-20 flex items-center gap-2 px-4 py-2 bg-gray-900/95 backdrop-blur border-b border-gray-800">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <UploadArea onLoad={handleLoad} compact />
              <button
                onClick={clearAll}
