    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-gray-900 border-l border-gray-700 z-50 flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {/* Contortion / Why */}
          {(entry.contortion_focus || entry.why_it_fits || entry.pose_setpiece || entry.plot_scaffold || entry.anatomical_breakdown || entry.pose_family) && (
            <div className="space-y-2">
              ...
              {entry.pose_setpiece && (
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">Key Pose Setpiece</p>
                  <p className="text-sm text-gray-300">{entry.pose_setpiece}</p>
                </div>
              )}
              {entry.plot_scaffold && (
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">Plot Scaffold</p>
                  <p className="text-sm text-gray-300">{entry.plot_scaffold}</p>
                </div>
              )}
              {entry.anatomical_breakdown && (
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">Anatomical Breakdown</p>
                  <p className="text-sm text-gray-300">{entry.anatomical_breakdown}</p>
                </div>
              )}
              {entry.pose_family && (
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">Pose Family</p>
                  <p className="text-sm text-gray-300">{entry.pose_family}</p>
                </div>
              )}
            </div>
          )}
